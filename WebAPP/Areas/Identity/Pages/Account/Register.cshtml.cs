        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    //aqui validamos si el rol exite para crearlo
                    if (!await roleManager.RoleExistsAsync(CNT.Admin))
                    {
                        await roleManager.CreateAsync(new IdentityRole(CNT.Admin));
                        await roleManager.CreateAsync(new IdentityRole(CNT.Usuarios));

                    }
                    //optener el rol seleccionado
                    ...
                    else
                    {
                        if (rol == CNT.Usuarios)
                        {
                            await _userManager.AddToRoleAsync(user, CNT.Usuarios);

                        }
                    }

                    _logger.LogInformation("Usuario Creado Con Exito");
                    //para aprovar el usuario en la base de datos
